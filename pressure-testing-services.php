<?php include('header.php'); ?>
<section class="wrapper image-wrapper bg-overlay bg-overlay-400 text-white innerPageBannerContainer">
    <div class="container pt-17 pb-17 pt-md-17 pb-md-10 text-center">
        <div class="row">
            <div class="col-12 mx-auto">
                <h1 class="display-1 mb-3 text-white">Pressure Testing Services</h1>
                <nav class="d-inline-block" aria-label="breadcrumb">
                    <ol class="breadcrumb text-white">
                        <li class="breadcrumb-item"><a href="#">Home</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Pressure Testing Services</li>
                    </ol>
                </nav>
                <!-- /nav -->
            </div>
            <!-- /column -->
        </div>
        <!-- /.row -->
    </div>
    <!-- /.container -->
</section>


<section>
    <div class="container">
        <div class="row gx-lg-8 gy-8 mt-5 mt-md-12 mt-lg-0 mb-15 align-items-center">
            <div class="col-lg-6 order-lg-2">
                <div class="row gx-md-5 gy-5">
                    <div class="col-md-4 offset-md-2 align-self-end">
                        <figure class="rounded"><img src="assets/esol/img/service-images/pressure-valve/pressure-test/atcs-service-top-left.png" srcset="assets/esol/img/service-images/pressure-valve/pressure-test/atcs-service-top-left.png 2x" alt=""></figure>
                    </div>
                    <!--/column -->
                    <div class="col-md-6 align-self-end">
                        <figure class="rounded"><img src="assets/esol/img/service-images/pressure-valve/pressure-test/atcs-service-top-right.png" srcset="assets/esol/img/service-images/pressure-valve/pressure-test/atcs-service-top-right.png 2x" alt=""></figure>
                    </div>
                    <!--/column -->
                    <div class="col-md-6 offset-md-1">
                        <figure class="rounded"><img src="assets/esol/img/service-images/pressure-valve/pressure-test/atcs-service-bottom-left.png" srcset="assets/esol/img/service-images/pressure-valve/pressure-test/atcs-service-bottom-left.png 2x" alt=""></figure>
                    </div>
                    <!--/column -->
                    <div class="col-md-4 align-self-start">
                        <figure class="rounded"><img src="assets/esol/img/service-images/pressure-valve/pressure-test/atcs-service-bottom-right.png" srcset="assets/esol/img/service-images/pressure-valve/pressure-test/atcs-service-bottom-right.png 2x" alt=""></figure>
                    </div>
                    <!--/column -->
                </div>
                <!--/.row -->
            </div>
            <!--/column -->
            <div class="col-lg-6 serviceTextContainer">
                <h2 class="display-4 mb-3">
                    Pressure Testing Services
                </h2>
                <p class="lead mb-8 pe-xxl-2">
                    Pressure Testing & Services involves checking the integrity, strength, and leak-tightness of equipment, pipelines, vessels, or systems that operate under pressure. This is essential in industries like oil & gas, petrochemical, water treatment, and manufacturing to ensure safety, compliance, and operational reliability.
                </p>
                <h4>
                    Methods:
                </h4>
                </h2>
                <p class="lead mb-8 pe-xxl-2">
                    Hydrostatic Testing: Uses water or another liquid to pressurize the system.<br>
                    Pneumatic Testing: Uses air or inert gas for pressurization.
                </p>
            </div>
            <!--/column -->
        </div>
    </div>
</section>

<section class="wrapper bg-soft-primary">
    <div class="container py-14 py-md-12">
        <h2 class="display-4 mb-5 text-center pb-3">
            We perform pressure testing on
        </h2>
        <div class="row gx-md-5 gy-5 mb-14 mb-md-17">
            <div class="col-md-6 col-xl-3">
                <div class="card shadow-lg card-border-bottom border-soft-yellow h-100">
                    <div class="card-body serviceCardCstm">
                        <h2>01</h2>
                        <!-- <img src="assets/esol/icons/capability.png" class="svg-inject icon-svg icon-svg-md text-yellow mb-3" alt="" /> -->
                        <h4>
                            All Types of Valves
                        </h4>
                    </div>
                    <!--/.card-body -->
                </div>
                <!--/.card -->
            </div>
            <!--/column -->
            <div class="col-md-6 col-xl-3">
                <div class="card shadow-lg card-border-bottom border-soft-green h-100">
                    <div class="card-body serviceCardCstm">
                        <h2>02</h2>
                        <h4>
                            Chiksan Lines
                        </h4>
                    </div>
                    <!--/.card-body -->
                </div>
                <!--/.card -->
            </div>
            <!--/column -->
            <div class="col-md-6 col-xl-3">
                <div class="card shadow-lg card-border-bottom border-soft-orange h-100">
                    <div class="card-body serviceCardCstm">
                        <h2>03</h2>
                        <h4>
                            Pipelines
                        </h4>
                    </div>
                    <!--/.card-body -->
                </div>
                <!--/.card -->
            </div>
            <!--/column -->
            <div class="col-md-6 col-xl-3">
                <div class="card shadow-lg card-border-bottom border-soft-blue h-100">
                    <div class="card-body serviceCardCstm">
                        <h2>04</h2>
                        <h4>Flexible Hoses</h4>
                    </div>
                    <!--/.card-body -->
                </div>
                <!--/.card -->
            </div>
            <!--/column -->
            <!--/column -->
            <div class="col-md-6 col-xl-3">
                <div class="card shadow-lg card-border-bottom border-soft-yellow h-100">
                    <div class="card-body serviceCardCstm">
                        <h2>05</h2>
                        <h4>
                            Silo Tanks
                        </h4>
                    </div>
                    <!--/.card-body -->
                </div>
                <!--/.card -->
            </div>
            <!--/column -->
            <!--/column -->
            <div class="col-md-6 col-xl-3">
                <div class="card shadow-lg card-border-bottom border-soft-orange h-100">
                    <div class="card-body serviceCardCstm">
                        <h2>06</h2>
                        <h4>Air Tanks</h4>
                    </div>
                    <!--/.card-body -->
                </div>
                <!--/.card -->
            </div>
            <!--/column -->
            <!--/column -->
            <div class="col-md-6 col-xl-3">
                <div class="card shadow-lg card-border-bottom border-soft-blue h-100">
                    <div class="card-body serviceCardCstm">
                        <h2>07</h2>
                        <h4>Pressure Vessels</h4>
                    </div>
                    <!--/.card-body -->
                </div>
                <!--/.card -->
            </div>
            <!--/column -->
        </div>
        <!--/.row -->

    </div>
    <!-- /.container -->
</section>


<section class="wrapper bg-light">
    <div class="container py-14 py-md-10">

        <div class="row mb-3">
            <div class="col-md-10 col-lg-12 col-xl-10 col-xxl-9 mx-auto text-center">
                <h3 class="display-4 mb-7 px-lg-19 px-xl-18">
                    Key Personnel
                </h3>
            </div>
            <!--/column -->
        </div>
        <!--/.row -->
        <div class="row grid-view gx-md-8 gx-xl-10 gy-8 gy-lg-0 mb-md-10 justify-content-center">
            <div class="col-lg-6">
                <div class="position-relative">
                    <div class="shape rounded bg-soft-primary rellax d-md-block" data-rellax-speed="0" style="bottom: -0.75rem; right: -0.75rem; width: 98%; height: 98%; z-index:0"></div>
                    <div class="card">
                        <div class="card-body px-6 py-5">
                            <h4 class="mb-1">Pravin Kumar</h4>
                            <p class="mb-0">Asst. Manager</p>
                            <div class="pt-3">
                                <div class="d-flex flex-row">
                                    <div>
                                        <div class="icon text-primary fs-23 me-4 mt-n1"> <i class="uil uil-phone-volume"></i> </div>
                                    </div>
                                    <div>
                                        <a href="tel:00974 77741597">
                                            <p>
                                                00974 77741597
                                            </p>
                                        </a>
                                    </div>
                                </div>
                                <div class="d-flex flex-row">
                                    <div>
                                        <div class="icon text-primary fs-23 me-4 mt-n1"> <i class="uil uil-envelope"></i> </div>
                                    </div>
                                    <div>
                                        <a href="mailto:calqa@ariesgroup.ae">
                                            <p>
                                                calqa@ariesgroup.ae
                                            </p>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!--/.card-body -->
                    </div>
                    <!-- /.card -->
                </div>
                <!-- /div -->
            </div>

        </div>
        <!--/.row -->
    </div>
    <!-- /.container -->
</section>

<?php include('footer.php'); ?>